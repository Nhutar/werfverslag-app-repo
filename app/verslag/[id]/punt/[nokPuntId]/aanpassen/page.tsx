"use client";

import { useEffect, useRef, useState } from "react";
import Link from "next/link";

interface Deelnemer {
  id: string;
  naam: string;
  discipline: string;
  email: string;
}

const MAX_FOTOS = 5;

export default function NokPuntAanpassenPagina({
  params,
}: {
  params: { id: string; nokPuntId: string };
}) {
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [laden, setLaden] = useState(true);
  const [verslagNaam, setVerslagNaam] = useState("");
  const [deelnemers, setDeelnemers] = useState<Deelnemer[]>([]);

  const [titel, setTitel] = useState("");
  const [omschrijving, setOmschrijving] = useState("");
  const [deelnemerId, setDeelnemerId] = useState("");
  const [deadline, setDeadline] = useState("");
  const [bestaandeFotoUrls, setBestaandeFotoUrls] = useState<string[]>([]);
  const [nieuweFotos, setNieuweFotos] = useState<File[]>([]);
  const [nieuwePreviews, setNieuwePreviews] = useState<string[]>([]);

  const [bezig, setBezig] = useState(false);
  const [fout, setFout] = useState<string | null>(null);

  useEffect(() => {
    fetch(`/api/nok-punten/${params.nokPuntId}`)
      .then((r) => r.json())
      .then((data) => {
        setVerslagNaam(data.verslagNaam ?? "");
        setDeelnemers(data.deelnemers ?? []);
        setTitel(data.titel ?? "");
        setOmschrijving(data.omschrijving ?? "");
        setDeadline(data.deadline ?? "");
        setBestaandeFotoUrls(data.fotoUrls ?? []);

        // Zoek de deelnemer die overeenkomt met de huidige verantwoordelijke
        const match = (data.deelnemers ?? []).find(
          (d: Deelnemer) => d.email === data.verantwoordelijkeEmail
        );
        setDeelnemerId(match?.id ?? (data.deelnemers?.[0]?.id ?? ""));
      })
      .finally(() => setLaden(false));
  }, [params.nokPuntId]);

  useEffect(() => {
    const urls = nieuweFotos.map((f) => URL.createObjectURL(f));
    setNieuwePreviews(urls);
    return () => urls.forEach((u) => URL.revokeObjectURL(u));
  }, [nieuweFotos]);

  function fotoToevoegen(e: React.ChangeEvent<HTMLInputElement>) {
    const gekozen = Array.from(e.target.files ?? []);
    const beschikbaar = MAX_FOTOS - bestaandeFotoUrls.length;
    const samengevoegd = [...nieuweFotos, ...gekozen].slice(0, beschikbaar);
    setNieuweFotos(samengevoegd);
    if (fileInputRef.current) fileInputRef.current.value = "";
  }

  function bestaandeFotoVerwijderen(url: string) {
    setBestaandeFotoUrls((prev) => prev.filter((u) => u !== url));
  }

  function nieuweFotoVerwijderen(index: number) {
    setNieuweFotos((prev) => prev.filter((_, i) => i !== index));
  }

  async function opslaan(e: React.FormEvent) {
    e.preventDefault();
    if (!deelnemerId) {
      setFout("Kies een verantwoordelijke.");
      return;
    }
    setBezig(true);
    setFout(null);

    const fd = new FormData();
    fd.append("titel", titel);
    fd.append("omschrijving", omschrijving);
    fd.append("deelnemerId", deelnemerId);
    fd.append("deadline", deadline);
    fd.append("bestaandeFotoUrls", JSON.stringify(bestaandeFotoUrls));
    nieuweFotos.forEach((f) => fd.append("fotos", f));

    const res = await fetch(`/api/nok-punten/${params.nokPuntId}`, {
      method: "PATCH",
      body: fd,
    });

    if (res.ok) {
      window.location.href = `/verslag/${params.id}`;
    } else {
      const data = await res.json().catch(() => ({}));
      setFout(data.error ?? "Er is iets misgegaan. Probeer opnieuw.");
      setBezig(false);
    }
  }

  const gekozenDeelnemer = deelnemers.find((d) => d.id === deelnemerId);
  const totaalFotos = bestaandeFotoUrls.length + nieuweFotos.length;

  if (laden) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <p className="text-sm text-gray-400">Laden...</p>
      </div>
    );
  }

  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <Link
        href={`/verslag/${params.id}`}
        className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-6"
      >
        ← Terug naar project
      </Link>

      <h1 className="text-2xl font-bold text-gray-900">{verslagNaam}</h1>
      <p className="text-sm text-gray-500 mb-6">NOK-punt aanpassen</p>

      <form onSubmit={opslaan} className="flex flex-col gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col gap-4">
          {/* Titel */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Titel <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={titel}
              onChange={(e) => setTitel(e.target.value)}
              maxLength={80}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>

          {/* Omschrijving */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Omschrijving
            </label>
            <textarea
              rows={3}
              value={omschrijving}
              onChange={(e) => setOmschrijving(e.target.value)}
              placeholder="Optionele details..."
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
            />
          </div>

          {/* Foto's */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Foto&apos;s{" "}
              <span className="text-gray-400 font-normal">
                ({totaalFotos}/{MAX_FOTOS})
              </span>
            </label>

            {/* Bestaande foto's */}
            {bestaandeFotoUrls.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-2">
                {bestaandeFotoUrls.map((url, i) => (
                  <div key={i} className="relative w-20 h-20">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={url}
                      alt={`Foto ${i + 1}`}
                      className="w-20 h-20 object-cover rounded-lg border border-gray-200"
                    />
                    <button
                      type="button"
                      onClick={() => bestaandeFotoVerwijderen(url)}
                      className="absolute -top-1.5 -right-1.5 bg-white border border-gray-300 rounded-full w-5 h-5 flex items-center justify-center text-xs text-gray-500 hover:text-red-500 hover:border-red-300 shadow-sm"
                    >
                      ✕
                    </button>
                  </div>
                ))}
              </div>
            )}

            {/* Nieuwe foto previews */}
            {nieuwePreviews.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-2">
                {nieuwePreviews.map((url, i) => (
                  <div key={i} className="relative w-20 h-20">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={url}
                      alt={`Nieuwe foto ${i + 1}`}
                      className="w-20 h-20 object-cover rounded-lg border border-blue-200 ring-1 ring-blue-300"
                    />
                    <button
                      type="button"
                      onClick={() => nieuweFotoVerwijderen(i)}
                      className="absolute -top-1.5 -right-1.5 bg-white border border-gray-300 rounded-full w-5 h-5 flex items-center justify-center text-xs text-gray-500 hover:text-red-500 hover:border-red-300 shadow-sm"
                    >
                      ✕
                    </button>
                  </div>
                ))}
              </div>
            )}

            {totaalFotos < MAX_FOTOS && (
              <>
                <input
                  ref={fileInputRef}
                  type="file"
                  accept="image/*"
                  capture="environment"
                  multiple
                  onChange={fotoToevoegen}
                  className="hidden"
                  id="foto-input"
                />
                <label
                  htmlFor="foto-input"
                  className="flex items-center justify-center gap-2 w-full border-2 border-dashed border-gray-300 rounded-lg p-4 text-sm text-gray-400 hover:border-blue-400 hover:text-blue-500 transition-colors cursor-pointer"
                >
                  📷 Foto toevoegen
                </label>
              </>
            )}
          </div>

          {/* Verantwoordelijke */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Verantwoordelijke <span className="text-red-500">*</span>
            </label>
            {deelnemers.length === 0 ? (
              <p className="text-sm text-orange-600 bg-orange-50 border border-orange-200 rounded-lg px-3 py-2">
                Geen deelnemers beschikbaar.
              </p>
            ) : (
              <>
                <select
                  value={deelnemerId}
                  onChange={(e) => setDeelnemerId(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  {deelnemers.map((d) => (
                    <option key={d.id} value={d.id}>
                      {d.naam} — {d.discipline}
                    </option>
                  ))}
                </select>
                {gekozenDeelnemer && (
                  <p className="text-xs text-gray-400 mt-1 px-1">
                    {gekozenDeelnemer.email}
                  </p>
                )}
              </>
            )}
          </div>

          {/* Deadline */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Deadline <span className="text-red-500">*</span>
            </label>
            <input
              type="date"
              required
              value={deadline}
              onChange={(e) => setDeadline(e.target.value)}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        </div>

        {fout && (
          <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
            {fout}
          </p>
        )}

        <div className="flex gap-3">
          <button
            type="submit"
            disabled={bezig}
            className="flex-1 bg-blue-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {bezig ? "Bezig met opslaan..." : "Opslaan"}
          </button>
          <Link
            href={`/verslag/${params.id}`}
            className="px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-800 transition-colors"
          >
            Annuleren
          </Link>
        </div>
      </form>
    </div>
  );
}
