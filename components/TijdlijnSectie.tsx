...
export function TijdlijnSectie({ verslagen, startdatum, projectNaam, verantwoordelijkeModus = false }: Props) {
  ...
  const [zoom, setZoom] = useState<ZoomNiveau>("7dagen");
  ...
  // Herbruikbare filter + toolbar sectie
  const toolbar = (
    <div className="flex gap-2 flex-wrap items-center justify-between">
      {/* Filters — enkel als er punten zijn */}
      {allePunten.length > 0 ? (
        <div className="flex gap-2 flex-wrap items-center">
          ...
          {filterActief && (
            <button onClick={() => { setFilterVerantwoordelijke(""); setFilterStatus("alle"); setFilterDiscipline("alle"); }}
              className="text-xs text-gray-500 hover:text-gray-700 underline">Wissen</button>
          )}
          {geselecteerdVerslagId && (
            <button onClick={() => setGeselecteerdVerslagId(null)}
      ...
      {/* Zoom + volledig scherm — altijd zichtbaar */}
      <div className="flex items-center gap-2 flex-shrink-0">
        <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5">
          {ZOOM_LABELS.map(({ waarde, label }) => (
            <button key={waarde} type="button" onClick={() => setZoom(waarde)}
        ...
        <button
          ...
          className="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors text-xs font-medium border border-blue-200 flex-shrink-0"
        >
          {volledigScherm ? "↙ Sluiten" : "↗ Volledig scherm"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      {volledigScherm && (
        <div className="fixed inset-0 z-40 bg-white flex flex-col p-4 gap-3">
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            <span className="text-sm font-semibold text-gray-700">{projectNaam} — Tijdlijn</span>
            <button onClick={() => setVolledigScherm(false)} className="text-gray-400 hover:text-gray-700 text-xl leading-none">✕</button>
          </div>
          {toolbar}
      ...
      {verslagModaal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4 py-8" onClick={() => setVerslagModaal(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between p-5 border-b border-gray-100">
              <div>
                <h2 className="text-base font-bold text-gray-900">
                  Werfverslag van {new Date(verslagModaal.datum).toLocaleDateString("nl-BE", { day: "numeric", month: "long", year: "numeric" })}
                </h2>
